...
metadata:
  short-description: "Brand assets: og.jpg card, X feed banner, SVG favicon, PWA icons — parallel task by default"
...
The identity bundle — `public/og.jpg`, `public/x-banner.jpg` (games),
`public/favicon.svg`, `site.json`, PWA icons when installable — needs only the
app's name, theme, and palette. **Start it as a background task with the `task`
tool** as soon as the name and palette settle (AGENTS.md § "Parallel work"): give it
this skill plus `references/custom-card.md` and sole ownership of `public/` brand
assets + `src/lib/og/site.json`, keep building, and merge `site.json` at the end
(`wait_tasks` before the final verify) — an inline card pass serializes minutes of
generation latency behind a finished app. Stay sequential when the user is
art-directing, when the art to reuse doesn't exist yet, or when `task` is absent.
